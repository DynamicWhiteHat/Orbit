...
void sendMavlink() {
	...
	if (t - lastSlow >= PERIOD_SLOW) {
		...
		sendMessage(&msg);
			...
			if (batteryData.voltage <= BATTERY_MAVLINK_STATUS_EMERGENCY)
			{
				batteryChargeState = MAV_BATTERY_CHARGE_STATE::MAV_BATTERY_CHARGE_STATE_EMERGENCY;
				setLED(0);
			}
			else if (batteryData.voltage <= BATTERY_MAVLINK_STATUS_CRITICAL)
			{
				batteryChargeState = MAV_BATTERY_CHARGE_STATE::MAV_BATTERY_CHARGE_STATE_CRITICAL;
				setLED(0);				
			}
			else if (batteryData.voltage <= BATTERY_MAVLINK_STATUS_LOW)
			{
			...
			const uint16_t voltages[10] = {
				batteryData.voltage, // changed from busVoltage
			...
			mavlink_msg_battery_status_pack(
				...
				-1,
				-1,
				map(constrain(batteryData.voltage, BATTERY_MIN_LEVEL, BATTERY_MAX_LEVEL), BATTERY_MIN_LEVEL, BATTERY_MAX_LEVEL, 0, 100),
				0,
				batteryChargeState,
				voltagesExt,
			...
			mavlink_msg_global_position_int_pack(
				...
				NULL,
				NULL,
				paToAltitude(barometerData.pa) * 1000,
				NULL,
				NULL,
				NULL,
				NULL,
	...
	if (t - lastFast >= PERIOD_FAST) {
		...
		mavlink_msg_scaled_imu_pack(
			...
			// Magnetometer (milli-Gauss, FLU → FRD)
			compassValues.x * 1000,
			-compassValues.y * 1000,
			-compassValues.z * 1000,
